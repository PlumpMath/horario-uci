...
import uci.horarioUCI.tools.CalendarManager;
...
public class AsyncTaskManHidden {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what != 1) { // code if not connected
				log("La aplicación no tiene acceso a la red.");
				Log.v("horario", "No hay ping");
			} else { // code if connected
				log("La aplicación tiene acceso a la red.");
	...
	private class VerificarConexionIntranetdAsyncTask extends
	...
	private class ObtenerHorarioIntranetdAsyncTask extends
		...
		protected String[][] doInBackground(Void... params) {
			try {
				...
				return horarioUCIParser.obtenerHorario(facultad, semana,
						brigada);
			} catch (Exception e) {
				log(e.getMessage());
				return new String[][] {};
		...
		protected void onPostExecute(String[][] result) {
			...
			if (result == null) {
				log("No existe horario para la brigada " + brigada
						+ " en la semana: " + semana);
				print("No existe horario para la brigada " + brigada
						+ " en la semana: " + semana);
				return;
			}
			if (result.length == 0) {
				log("No se pueden obtener los datos del horario en este momento. Inténtelo más tarde");
				print("No se pueden obtener los datos del horario en este momento. Inténtelo más tarde");
				return;
			}
			...
			db.insertarHorarioBD(facultad, semana, brigada, cadenaHorario,
					fecha);
			String[] horario = new String[] { semana, facultad, brigada,
					cadenaHorario, fecha };
			log("Se ha guardado el horario para la brigada " + brigada
					+ " en la semana: " + semana);
			actualizarDatos(horario);
			print("** Se actualizo el horario **");

			new NotificationTools().showNotification(context, "HorarioUCI",
					"Se actualizó el horario - " + new Date().toLocaleString());
		...
		private String convertirFecha(Date fechaDate) {
			boolean am = Integer.parseInt(fechaDate.toString().split(" ")[3]
					.split(":")[0]) < 12;
